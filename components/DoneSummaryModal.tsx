...
export function DoneSummaryModal({
  ...
  if (!calorieExceeded && proteinTargetMet) {
    ...
    feedbackEmoji = "🌟";
    feedbackBadgeColor = "bg-[#1DB954]/10 border-[#1DB954]/30 text-[#1DB954] shadow-[0_0_12px_rgba(29,185,84,0.15)]";
    ...
    feedbackEmoji = "🍗";
    feedbackBadgeColor = "bg-orange-500/10 border-orange-500/30 text-orange-400";
  } else if (!calorieExceeded && !proteinTargetMet) {
  ...
  return (
    <div className="fixed inset-0 z-55 flex items-center justify-center p-4 bg-black/80 backdrop-blur-md animate-fade-in">
      <div className="w-full max-w-sm border border-white/10 bg-[#181818]/95 backdrop-blur-xl p-6 shadow-2xl rounded-3xl relative animate-slide-up">
        {/* Close button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 text-zinc-400 hover:text-white p-1 hover:bg-zinc-800/80 transition rounded-full cursor-pointer"
        ...
        <div className="text-center mb-6">
          <div className="mx-auto flex h-12 w-12 items-center justify-center bg-[#282828] text-2xl mb-3 border border-white/5 rounded-full">
            {feedbackEmoji}
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 mb-6">
          {/* Calorie Stats Card */}
          <div className="glass-panel-accent p-4 rounded-2xl flex flex-col justify-between border border-white/5 bg-[#282828]/20">
            <div>
              <span className="text-[10px] font-bold tracking-widest text-zinc-400 uppercase font-sans">
                Calories Eaten
              </span>
              <p className="text-xl font-extrabold tracking-tight text-white font-sans mt-1">
            ...
            <div className="mt-4">
              ...
              <div className="h-2 w-full bg-zinc-800 rounded-full overflow-hidden mb-2">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${
                    calorieExceeded
                      ? "bg-[#FF5252] shadow-[0_0_8px_rgba(255,82,82,0.4)]"
                      : "bg-[#F97316]"
                  }`}
              ...
              {calorieExceeded ? (
                <span className="text-[10px] font-bold text-[#FF5252] font-sans flex items-center gap-0.5">
                  ⚠️ {calorieDiff.toLocaleString()} over
                </span>
              ) : (
                <span className="text-[10px] font-bold text-[#1DB954] font-sans flex items-center gap-0.5">
            ...
            <div className="mt-4">
              ...
              <div className="h-2 w-full bg-zinc-800 rounded-full overflow-hidden mb-2">
                <div
                  className={`h-full rounded-full transition-all duration-500 ${
                    proteinTargetMet
                      ? "bg-[#1DB954] shadow-[0_0_8px_rgba(29,185,84,0.4)]"
                      : "bg-[#1DB954]/60"
                  }`}
        ...
        <div className={`p-4 rounded-2xl border ${feedbackBadgeColor} mb-6 transition-all duration-300`}>
          <h3 className="text-xs font-extrabold uppercase tracking-wider font-sans mb-1 flex items-center gap-1.5">
            {feedbackTitle}
          </h3>
          <p className="text-xs font-semibold leading-relaxed text-zinc-200 font-sans">
            {feedbackMessage}
          </p>
        </div>
        ...
        <div className="space-y-2 mb-6 border-t border-white/5 pt-4">
          <div className="flex items-center justify-between text-xs font-bold font-sans">
            ...
            {calorieExceeded ? (
              <span className="text-[#FF5252] bg-[#FF5252]/10 border border-[#FF5252]/20 px-2.5 py-0.5 rounded-full">
                Surplus Exceeded
              </span>
            ) : (
              <span className="text-[#1DB954] bg-[#1DB954]/10 border border-[#1DB954]/20 px-2.5 py-0.5 rounded-full">
                Under Budget ✓
              </span>
            )}
          </div>
          ...
          <div className="flex items-center justify-between text-xs font-bold font-sans">
            ...
            {proteinTargetMet ? (
              ...
              <span className="text-orange-400 bg-orange-500/10 border border-orange-500/20 px-2.5 py-0.5 rounded-full">
                Remaining Target
              </span>
            )}
          </div>
        ...
        <button
          onClick={onClose}
          className="w-full bg-[#1DB954] hover:bg-[#1ed760] py-3 text-sm font-extrabold text-black transition active:scale-95 rounded-full shadow-md cursor-pointer text-center"
        >
          Got it, back to logs
        </button>
      </div>
